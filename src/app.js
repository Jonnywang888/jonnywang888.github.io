...
class WolApp {
    ...
    static CARD_COLORS = [
        { id: 'blue',    gradient: 'linear-gradient(135deg, #1A73E8 0%, #0D47A1 100%)', pattern: '#fff',stato :'#000'},
        { id: 'purple',  gradient: 'linear-gradient(135deg, #7B1FA2 0%, #4A148C 100%)', pattern: '#fff',stato :'#000'},
        { id: 'teal',    gradient: 'linear-gradient(135deg, #00796B 0%, #004D40 100%)', pattern: '#fff',stato :'#000'},
        { id: 'indigo',  gradient: 'linear-gradient(135deg, #3949AB 0%, #1A237E 100%)', pattern: '#fff',stato :'#000'},
        { id: 'rose',    gradient: 'linear-gradient(135deg, #C62828 0%, #880E4F 100%)', pattern: '#fff',stato :'#000'},
        { id: 'orange',  gradient: 'linear-gradient(135deg, #E65100 0%, #BF360C 100%)', pattern: '#fff',stato :'#000'},
        { id: 'green',   gradient: 'linear-gradient(135deg, #2E7D32 0%, #1B5E20 100%)', pattern: '#fff',stato :'#000'},
        { id: 'slate',   gradient: 'linear-gradient(135deg, #37474F 0%, #1C313A 100%)', pattern: '#fff',stato :'#000'},
    ];
    ...
    _loadDevices() {
        ...
        if (!device.stato) device.stato = 'off';
        if (!device.sid) device.sid = '';
        }
    }
    ...
    _buildCard(d) {
        ...
        <div class="pc-card-bg">
                ...
                <!-- Monitor outline -->
                <rect x="55" y="60" width="90" height="62" rx="4" fill="none"
                        stroke="${color.pattern}" stroke-width="1.5" opacity="0.35"/>
                <!-- Scre -->
                <rect x="60" y="65" width="80" height="52" rx="2" fill="${color.stato}" opacity="0.60"/>
                <!-- Stand -->
                <rect x="91" y="122" width="18" height="10" rx="1" fill="none"
                        stroke="${color.pattern}" stroke-width="1.5" opacity="0.35"/>
                <rect x="80" y="132" width="40" height="4" rx="2" fill="none"
                        stroke="${color.pattern}" stroke-width="1.5" opacity="0.35"/>
                <!-- Decorative circles -->
                ...
                <!-- Screen lines -->
                <line x1="66" y1="76" x2="114" y2="76" stroke="${color.pattern}" stroke-width="1" opacity="0.25"/>
                <line x1="66" y1="84" x2="104" y2="84" stroke="${color.pattern}" stroke-width="1" opacity="0.20"/>
                <line x1="66" y1="92" x2="110" y2="92" stroke="${color.pattern}" stroke-width="1" opacity="0.20"/>
                </svg>
        </div>
            ...
            <div class="pc-card-bottom">
                <div class="pc-card-wake-row">
                    <div class="wake-mini-btn" data-action="wake" data-id="${d.id}"
                            ${isBusy ? 'disabled' : ''} aria-label="开机 ${this._esc(d.name)}">
                    ${isBusy
    ...
    _isconfigured(id) {
        const d = this._getDevice(id);
        if (!d) return false;
        return d.baseUrl && d.fritzUrl && d.username && d.password && d.deviceUrl;
    }
    /* ══════════════════════════════════════════════════════
    ...
    async _runWake(id) {
        ...
        if (!this._isconfigured(id)) {
            this._log('请先配置设备参数', 'error', id);
            return;
        }
        const {baseUrl, fritzUrl, username, password, deviceUrl, sid} = d;
        this.busyIds.add(id);
        this._updateCardBusy(id, true);
        try {
            if (sid === '') {
                this._log('── 开始认证 ──', '', id);
                sid = await this._getSid(baseUrl, fritzUrl, username, password, id);
            }
            this._log('── 发送 WOL 指令 ──', '', id);
            const r = await this._fetchUrl(baseUrl, deviceUrl, 'WOL', sid, id);
            if (r.trim() == '{}') {
                this._log('响应内容: ' + (r.trim() || '(空)'), 'success', id);
                this._log('✓ 开机指令发送成功！', 'success', id);
                setTimeout(() => {
                    this._isonline(id);
                }, 30000); // 5秒后检查状态，给设备一些时间开机
                showmsg('开机成功 ✓');
            } else {
                this._log('响应内容: ' + (r.trim() || '(空)'), 'error', id);
                this._log('✗ 开机指令发送失败！可能是SID过期了,请重试！', 'error', id);
                sid = ''; // 可能是 SID 过期了，清掉让下一次重新认证
            }
        } catch (e) {
    ...
    _checkOnlineStatus() {
        this.devices.forEach(d => {
            if (this._isconfigured(d.id)) {
                this._isonline(d.id);
            }
        });
    }
    async _isonline(id) {
        const isconfig = this._isconfigured(id);
        if (!isconfig) return false;
        const d = this._getDevice(id);
        let {baseUrl, fritzUrl, username, password, deviceUrl,sid} = d;
        if (sid === '') {
            try {
                sid = await this._getSid(baseUrl, fritzUrl, username, password, id);
                d.sid = sid; // 保存 SID 以供后续使用
            } catch {
                this._log('认证失败，无法获取在线状态', 'error', id);
                d.stato = 'off';
                return false;
            }
        };
        const uid = deviceUrl.split('/').pop();
        const apiurl = deviceUrl.replace('/' + uid, '') 
        const r = await this._fetchUrl(baseUrl, apiurl, 'OLINE', sid, id);
    ...
    _clearLog() {
        ...
        '<div class="log-empty" id="logEmpty">暂无记录，点击卡片上的「开机」开始</div>';
    }
